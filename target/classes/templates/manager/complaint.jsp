...
<div class="xt-center">
    <div id="xt-right">
        <div class="xt-bt">功能导航> 投诉处理 > 最新投诉</div>
        <%
            ...
            String pages=request.getParameter("cpage");
            int p = Integer.parseInt(pages);
            List list=(List)request.getSession().getAttribute("complaint");
            if(list==null){
        %>
        <div class="xt-table">
        ...
        <div class="xt-fenye">
            ...
            <div class="xt-fenye-right">
                <a href="http://localhost:8080/manager/complaint.jsp?cpage=1">首页</a>
                <a href="http://localhost:8080/manager/complaint.jsp?cpage=1">上一步</a>
                <a href="http://localhost:8080/manager/complaint.jsp?cpage=1">下一步</a>
                <a href="http://localhost:8080/manager/complaint.jsp?cpage=1">尾页</a>
                <input type="text" name="text" />
                <a href="http://localhost:8080/manager/complaint.jsp?cpage=1" class="xt-link">跳转</a>
            </div>
        </div>
        <%}else{%>
        <div class="xt-table">
            <table cellpadding="0" cellspacing="0" border="0" bgcolor="#dcdcdc" width="100%">
                ...
                <%
                    ...
                    for(int i=0;i<((list.size()-10*(p-1))<10?(list.size()-10*(p-1)):10);i++){
                        Complaint complaint=(Complaint) list.get(i+(p-1)*10);
                %>
                     <tr class="xt-bg">
                         <td><%=i+(p-1)*10+1%></td>
                         ...
                         <td><a href="javascript:void (0);" onclick="agreecom(<%=complaint.getComid()%>);" class="blue-xt">投诉通过</a>
                         <td><a href="javascript:void (0);" onclick="disagreecom(<%=complaint.getComid()%>);" class="yellow-xt">投诉驳回</a>
                     </tr>
                <%}%>
            </table>
        </div>
        ...
        <div class="xt-fenye">
            ...
            <div class="xt-fenye-right">
                <a href="complaint.jsp?cpage=1">首页</a>
                <a href="http://localhost:8080/manager/complaint.jsp?cpage=<%if(p==1){%><%=1%><%}else{%><%=p-1%><%}%>">上一步</a>
                <a href= "javascript:void (0);" onclick="next();">下一步</a>
                <a href="complaint.jsp?cpage=<%=totalpages%>">尾页</a>
...
<script type="text/javascript">
    function next(){
        window.location.href="http://localhost:8080/manager/complaint.jsp?cpage=<%if(p>=totalpages){%><%=totalpages%><%}else{%><%=p+1%><%}%>";
    }
</script>
<script type="text/javascript">
    function jump(){
        ...
        window.location.href="http://localhost:8080/manager/complaint.jsp?cpage="+jumpto;
    }
</script>
